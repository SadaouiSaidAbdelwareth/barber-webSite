...
const Testimonials: React.FC = () => {
    ...
    useEffect(() => {
        const timer = setInterval(nextTestimonial, 6000); // Change testimonial every 6 seconds
    ...
    const currentTestimonial = testimonialItems[currentIndex];
    ...
    return (
        <section id="testimonials" className="py-20 bg-black text-white">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center flex flex-col items-center justify-center min-h-[50vh]">
                <div className="mb-8">
                    <h2 className="text-5xl md:text-6xl font-serif font-black uppercase tracking-wider">{t('testimonials.mainTitle')}</h2>
                    <div className="flex items-center justify-center mt-2">
                        <div className="h-px w-16 bg-brand-gold"></div>
                        <p className="mx-4 text-lg font-sans uppercase tracking-widest">{t('testimonials.subtitle')}</p>
                    </div>
                </div>
                    ...
                    {testimonialItems.map((testimonial: any, index: number) => (
                         <div key={index} className={`absolute w-full transition-opacity duration-1000 ease-in-out ${index === currentIndex ? 'opacity-100' : 'opacity-0'}`}>
                            ...
                            <div className="mt-8 flex flex-col items-center">
                                <img src={testimonial.imageUrl} alt={testimonial[language as Language].name} className="w-20 h-20 rounded-full object-cover mb-4 border-4 border-brand-gold shadow-xl" />
                                <cite className="text-xl font-bold text-white not-italic">{testimonial[language as Language].name}</cite>
                            </div>
                        </div>
                    ))}
                </div>
